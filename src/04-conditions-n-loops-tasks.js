...
function findFirstSingleChar(str) {
  ...
  const nonRepeatedChar = charArray.find((char) => {
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const intervalStr = `${startSymbol}${start}, ${end}${endSymbol}`;
  return intervalStr;
}
...
function isBracketsBalanced(str) {
  ...
  function checkBrackets(index, stack) {
    if (index === str.length) {
    ...
    return checkBrackets(index + 1, stack);
  }
  return checkBrackets(0, []);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const commonPath = firstPath
    .split('/')
    .reduce((acc, dir, index) => {
      const isCommon = restPaths.every((path) => path.split('/')[index] === dir);
      return isCommon ? `${acc}${dir}/` : acc;
    }, '');
  return commonPath;
}
